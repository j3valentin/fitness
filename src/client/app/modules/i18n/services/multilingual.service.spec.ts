...
import { CoreModule, WindowService, ConsoleService } from '../../core/index';
import { TEST_CORE_PROVIDERS, WindowMockSpanish } from '../../core/testing/index';
  ...
  t.describe('i18n:', () => {
    t.describe('MultilingualService', () => {
      ...
      t.it('changeLang - should not switch unless supported', t.inject([MultilingualService, Store], (multilang: MultilingualService, store: Store<any>) => {
        store.dispatch(new ChangeAction('sp'));
    ...
    t.describe('MultilingualService for Spanish browser/platform', () => {

      t.be(t.async(() => {
        testModuleConfig({
          window   : WindowMockSpanish,
          languages: [
            {code: 'en', title: 'English'},
      ...
      t.it('should now support spanish by default', t.inject([MultilingualService, Store, WindowService, Languages], (multilang: MultilingualService, store: Store<any>, win: WindowService, languages) => {
        t.e(languages.length).toBe(2);
        t.e(languages[0].code).toBe('en');
        t.e(languages[1].code).toBe('es');
        t.e(win.navigator.language).toBe('es-CO');

        store.select('i18n').subscribe((i18n: IMultilingualState) => {
          t.e(i18n.lang).toBe('es');
        });
      }));
    });
  });
